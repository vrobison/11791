...
public abstract class AbstractRanker implements IRanker {
  ...
  public List<Passage> rank(Question question, List<Passage> passages) {
    ...
    for(int i =0;i< passages.size();i++){
      Passage p = (Passage)passages.get(i).clone();
      p.setScore(score(question, passages.get(i)));
      rankedPassages.add(i, p);
      
    }
    //sort passages
    Collections.sort(rankedPassages, new PassageSort());
    Collections.reverse(rankedPassages);//descending order
